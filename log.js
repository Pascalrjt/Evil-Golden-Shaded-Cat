...
const EventLog = (() => {
  ...
  function summaries() {
    ...
    for (const e of entries) {
      if (!e.attempt_id || e.mode === "technical" || e.mode === "preview" || e.study_prepared === false) continue;
      if (!attempts.has(e.attempt_id)) attempts.set(e.attempt_id,{mode:e.mode || "study",session_role:e.session_role,prototype_version:e.prototype_version,session_id:e.session_id,attempt_id:e.attempt_id,attempt_kind:e.attempt_kind,participant_id:e.participant_id,scenario_id:e.scenario_id,location:e.location,group:e.group,variant:e.variant,order_position:e.order_position,think_aloud:e.think_aloud,started:false,task_outcome:"unstarted",confirmation_occurred:false,chosen_pickup_suitability:"no_confirmed_choice",observations:[]});
    ...
    for (const row of rows) if (SCENARIOS[row.scenario_id]?.study && !SCENARIOS[row.scenario_id]?.driverInterview) sessions.set(row.session_id,row);
    for (const row of sessions.values()) {
      (STUDY_GROUPS[row.group] || []).forEach((scenario_id,index)=>{
        ...
        rows.push({mode:"study",session_role:"passenger",prototype_version:row.prototype_version,session_id:row.session_id,attempt_id:null,attempt_kind:"planned",participant_id:row.participant_id,scenario_id,location:scenario.location,group:row.group,variant:scenario.variant,order_position:index+1,think_aloud:false,started:false,task_outcome:"unstarted",confirmation_occurred:false,chosen_pickup_suitability:"no_confirmed_choice",observations:[]});
      });
    }
  ...
  function summaryCSV() {
    return csv(summaries(),["mode","session_role","prototype_version","session_id","attempt_id","attempt_kind","participant_id","scenario_id","location","group","variant","order_position","think_aloud","started","started_at","ended_at","task_outcome","reviewed_task_outcome","outcome_review_note","confirmation_occurred","chosen_spot_id","chosen_label","chosen_pickup_suitability","decision","stop_reason","abandoned","skipped","skip_stage","skip_reason","p4_comparison_incomplete","elapsed_ms","wall_elapsed_ms","script_pause_ms","assistance_count","report_count","report_attempt_count","p4_report_flag","p3_report_accepted","p3_requirements_met","practice","chosen","displayed","observations","first_action","dispatch_decision","request_accepted","report_results","passenger_contacted","alternative_agreed","passenger_response","pending_pickup_id","workflow_stage","interview_context","active_pickup","outcome_recorded_by"]);
  }
